...
public class MonitoringManager {
	...
	/**
	 * Questo metodo crea il singleton di MonitoringManager
	 * 
	 * @param dbmng il riferimento a MonitoringManager
	 * @return il singleton di tipo MonitoringManager
	 */
	...
	/**
	 * Questo metodo permette di individuare la chiamata di consonante che ha
	 * permesso di ottenere la maggiore quantita' di punti
	 *
	 * @return la stringa contenente: il nickname del giocatore, la consonante
	 *         chiamata e la frase associata
	 */
	public String getBestMove() {
		MovesDTO bestMove = dbManager.getBestMove();
		String result;
		if (bestMove != null) {
			result = "" + bestMove.getPlayer().getNickname() + " " + bestMove.getMoveType() + " "
					+ bestMove.getManche().getPhrase().getPhrase();
			return result;
		} else
			return "- - -";
	}
	...
	/**
	 * Questo metodo permette di individuare la media di mosse necessarie per
	 * indovinare la soluzione
	 *
	...
	/**
	 * Questo metodo permette di individuare le statistiche relative ad un singolo
	 * giocatore
	 *
	 * @param nickname il nickname del giocatore
	 * @return una stringa contenente, divisi da spazi: numero manche giocate,
	 *         numero partite giocate, numero manche osservate, numero match
	 *         osservati, numero manche vinte, numero match vinti, punteggio medio
	 *         vinto per partita, numero medio di volte che ha dovuto cedere il
	 *         turno di gioco per manche, numero medio di volte che ha dovuto cedere
	 *         il turno di gioco per match, numero medio di volte che ha perso tutto
	...
	public String perPlayerStats(String nickname) {
		...
		if (user != null) {
			...
			String result = "" + numManchePlayed + " " + numMatchPlayed + " " + numMancheObserved + " "
					+ numMatchObserved + " " + numMancheWon + " " + numMatchWon + " " + averageScore + " "
					+ averagePassedTurnPerManche + " " + averagePassedTurnPerMatch + " " + averageLossPerManche + " "
					+ averageLossPerMatch;
			return result;
		} else
			return null;
	 ...
	 *
	 * @return una stringa contenente: i nickname dei giocatori che: detengono il
	 *         punteggio piu' alto per manche, detengono il punteggio piu' alto per
	 *         partita, che ha giocato piu' manche in assoluto, con la media di
	 *         punti acquisiti per manche piu' alta, che ha ceduto il turno piu'
	 *         volte a causa di errori, che ha perso tutto il maggior numero di
	 *         volte
	 */
