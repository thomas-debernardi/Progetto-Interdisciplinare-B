...
public class MatchWinnersDAOImpl implements MatchWinnersDAO {
	...
	MatchWinnersDAOImpl(Connection c) {
		con = c;
	}

	public boolean addMatchWinner(String idMatch, String idPlayer, int amount) throws SQLException {
		String queryAdd = "INSERT INTO " + matchWinnersTable + "(" + matchWinnersIdMatchAttribute + ","
				+ matchWinnersidPlayerAttribute + "," + matchWinnersAmountAttribute + ") " + "VALUES ('" + idMatch
				+ "','" + idPlayer + "'," + amount + ");";
	...
	public int getWonMatchesByUser(String id) throws SQLException {
		String queryGet = "SELECT COUNT(*) AS count FROM " + matchWinnersTable + " WHERE "
				+ matchWinnersidPlayerAttribute + " = '" + id + "';";
		Statement stmt = con.createStatement();
